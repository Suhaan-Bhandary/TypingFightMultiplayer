...
async function textGenerator() {
  ...
  if (sentenceText === undefined || sentenceText.length === 0) {
    // generating a random number for choosing a text.
    let randomIndex = Math.floor(Math.random() * offlineSentences.length);
...
async function getRandomTextThroughApi() {
  ...
  try {
    ...
    console.log('Error in getRandomTextThroughApi', error);
  }
}
